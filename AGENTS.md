...
├─ controls.py       the Scanpath rail's plot controls (presets + palette, then collapsible 👁️ Fixations / ↗️ Saccades / 📄 Stimulus / 🔥 Overlays sections — each `toggle → ⚙️ style → 🧹 filter` — and merged 📐 Figure & canvas) + column-mapping override UI + trial-filter panel
