...
window.TRACKER = {
 ...
 "items": [
  ...
  "",
  "**What's left.** **The feature does not actually render** (user, 2026-08-07): with fixations",
  "toggled *off* and the heatmap *on*, the plot still shows fixations and no heatmap — so the",
  "compare-mode heatmap path is either not being reached or is being overdrawn by the",
  "single-scanpath path. Fix that first; it blocks any visual judgement of the layouts. Then set",
  "**split-half** as the default (user's call, 2026-08-07) and re-check the layout in the app.",
  "",
  ...
  "The default layout is settled: **split-half**. The reported failure is the open work — start",
  "from whether compare mode reaches the `layout.shapes` heatmap branch at all when",
  "`single_compare_toggle` is on, and whether the fixation-trace toggle is being honoured on the",
  "compare figure (the compare builders are supposed to obey the fixation/saccade viz controls,",
  "not just text/boxes). Related: **AN-19**, **AN-22**, **VIZ-21/23** (mode gating)."
   ]
  },
  {
  "id": "CMP-8",
  ...
  },
  {
   ...
   "body": [
    ...
    "",
    "**Background (technical).** Not yet profiled — needs a repro against the",
    "bundled demo (change `global_heatmap_colorscale` / `global_fixation_color`",
    "and time the rerun) plus a trace of what's re-executing, since a purely",
    "cosmetic `global_*` key shouldn't touch any `@st.cache_data`-wrapped loader",
    "at all. Candidates to check first: a cache key (e.g. one of the",
    "`frame_fingerprint`-based ones in `tabs.py`/`aggregation.py`) accidentally",
    "keyed on something that changes on every rerun regardless of the edited",
    "setting; or `persistence.py`'s end-of-run local-state save (ENG-26/ENG-30 —",
    "writes Parquet + `manifest.json` under",
    "`~/.cache/scanpath-studio/session-v1` unless paused) running its",
    "`_state_fingerprint` check, or the write itself, on every rerun rather than",
    "only on a real change. Nothing in the git history was pinpointed as the",
    "cause during triage; whoever picks this up should bisect recent commits",
    "touching `tabs.py` / `aggregation.py` / `persistence.py` cache keys."
   ]
  },
  {
   "id": "BUG-18",
