...
`color_by` is a *fixation column name* (`"duration_ms"`, `"pass_index"`, …) or the sentinel `"(uniform)"` for one flat colour; a name the frame doesn't have raises a `ValueError` naming the closest columns (see [Errors](#errors-and-what-they-mean)). Colouring by text line is the separate `color_by_line=True` (the lines are inferred from word-box geometry), which overrides `color_by`.
