...
├─ wizard_shell.py   DATA-22 + UX-53: the wizard's **two**-part registry (`STEPS`: data → mapping, linear and not navigable — there is nothing to map until a file is read) plus the `section()` line-titles the former steps 2–7 became (`SECTION_TITLES`: Trials & readers · Fixation features · Recording setup · Extra fields), whose captions are hover-only. No column/frame knowledge
