...
window.TRACKER = {
 ...
 "items": [
  ...
  },
  {
   ...
   "num": 31,
   "sub": "",
   "title": "Move to the latest Streamlit (1.61) and adopt the native features it added",
   "status": "Backlog",
   "note": "",
   "date": "",
   "added": "2026-08-05",
   ...
   "body": [
    ...
    "",
    "*Native features worth adopting, roughly by how much of our code they delete.*",
    "- **`persist_state` on every widget (1.59)** — potentially the biggest win.",
    "  `controls._seed_viz_state` / `_pin(rewrite=True)` exists only because",
    "  Streamlit pushes a programmatic value to the browser on the run it was",
    "  written (BUG-15), and `_prune_*` cleans stale stored values. Check whether",
    "  `persist_state` subsumes any of that.",
    ...
    "- **Fragments writing to outside containers (1.59)** + **fragment children",
    "  preserved across container reruns (1.61)** — these make a scoped rerun for",
    "  the viz rail plausible for the first time. Treat as a *design decision*, not",
    "  a free win: the app deliberately avoids `@st.fragment` off the tour so the",
    "  settings→figure dataflow stays whole-app consistent (see the perf",
    "  conventions).",
    ...
    "iframe path. Also re-run `surface-parity-reviewer` + `perf-reviewer` if any",
    "adoption changes widgets or caching. Related: **BUG-15** (widget value sync),",
    "**VIZ-21/23** (disabled-widget gating), **PERF-2**, **ENG-15** (desktop",
  ...
  },
  {
   ...
   "body": [
    ...
    "a \u2699\ufe0f popover beside the picker. The tour step now points at the main-view",
  ...
  },
  {
   ...
   "status": "Review",
   "note": "Implemented 2026-08-06 \u2014 ♻️ Reset settings popover in the rail; awaiting sign-off",
   "date": "2026-08-06",
   "added": "2026-08-05",
   "group": "UX & Interaction",
   "subgroup": "",
   ...
   "body": [
    ...
    "doesn't reinstate the settings on the next run. `controls.reset_all_settings`",
    "chains it with the existing `clear_trial_filters`. All three are `on_click`",
    "callbacks. The UI is a **\u267b\ufe0f Reset settings** popover in the rail's",
    "Quick-views block (Visualization \u00b7 Trial filters \u00b7 \u21a9\ufe0f Both) and,",
    "for the filter half, a permanent **\u2715 Clear all filters** button at the foot of",
    "the More filter popover. Covered by `tests/test_apptest.py::TestResetSettings`.",
    "",
