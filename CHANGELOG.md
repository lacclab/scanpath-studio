...
#### Changed
- **A dataset loads about three times faster, in a seventh of the memory** (PERF-6) — an EyeLink IA report ships 174 columns and a fixation report 300; the column mapping and the optional-field registry claim 27 and 17 of them, and `normalize_*` dropped the rest on the next line. Parsing them anyway was the single largest cost of opening a dataset — and the reason loading the full OneStop reports reached ~25 GB resident before a single measure was computed. The read is now *planned* from the file's header (`data.plan_table_read` → `ReadPlan` → `data.read_mapped_table`): the column names are read without parsing a row, the mapping is proposed from those names, and only the columns that survive normalization are parsed. The same plan tells the reader that EyeLink's bare `.` means *missing* — but **only in columns the registry declares numeric**, never in a text column, where a word whose text is "." is a word — which removes the 22 `to_numeric` re-parses that were 72% of `normalize_words`. Measured on the full OneStop L1 Paragraph reports (360 readers, 24,046 trials, 2.63 M word rows, 2.40 M fixation rows): **~225 s and ~25 GB → 70 s and 3.6 GB**, with `normalize_words` alone going 13.0 s → 5.3 s. The normalized frames are unchanged — column for column, value for value, dtype for dtype, which is what `tests/test_column_pruning.py` pins. Wired into the OneStop server bundle (both the per-pid Parquet shards and the full `.csv.zip` reports) and the public OneStop reports; the upload wizard still reads whole files, because it offers the unclaimed columns as filter fields and has to show them first. — ENG-32 had parked this decision with its reasoning written down and two triggers to revisit on; both fired, so the move happened. The 26 live items are issues #105–#130, titled `[VIZ-37] …` so that every `#ID` already in `AGENTS.md`, `CLAUDE.md`, the `plans/` notes and the git history still resolves — GitHub's own numbering is treated as an implementation detail, and `tracker/migrated.json` is the ID → issue map. The house workflow moved with them rather than being replaced by GitHub's defaults, and it moved onto GitHub's *structured* fields rather than onto labels: the six statuses and the priorities are single-select columns on the [Scanpath Studio board](https://github.com/orgs/lacclab/projects/5), kind is the native issue type (Bug / Feature / Task), and only `area:*` — which fixes the ID prefix — and `waiting-on-you` stayed labels, because GitHub has no field for either. The stock `bug` / `enhancement` / `duplicate` labels went too, as duplicates of the issue types and the close reasons. **Closing an issue is the sign-off**, so an implemented item still lands in *Review*, open, until the user approves it, with its open calls in a `### ⚖ Waiting on you` checklist. The 320 closed items were deliberately **not** imported: their value is the write-up, not the workflow, so `tracker/data.js` keeps them and `tracker/server.py` still serves them at `http://127.0.0.1:8765/tracker/` — as a static site now, with the write API, the state validation and the whole editor gone (441 → 82 lines), because a frozen archive that can still be written to is one that drifts from the issues that replaced it. `tracker/to_github_issues.py` is the migration itself, kept as the record of how it was made. Issue templates put outside bug reports and feature requests into the same four-section shape.
