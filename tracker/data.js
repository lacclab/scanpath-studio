 ...
 "items": [
  ...
  },
  {
   ...
   "body": [
    ...
    "",
    "*Skipped, with the reason.* **`st.tabs(height=)`** — a fixed panel height",
    ...
    "sake. **`App.run()`** — the one genuine remainder: `cli.launch_app` /",
    "`desktop/launcher.py` shell into `streamlit run` with injected theme +",
    "usage-stats flags (BUG-6, DATA-12) that would have to be re-expressed, and it",
    "touches the PyInstaller bundle, so it needs a real desktop build + ",
    "`desktop/smoke_test.py` run to be worth doing — more than this item should",
    "carry.",
    "",
    ...
    "does — the participant is passed with the trial id). *(b)* Your review of the",
    "two judgement calls above (fragments, sidebar lock), since both were your items",
    "and both came back \"no\". And `App.run()` stays open by choice, above.",
    "",
    "**Background (technical).**",
    "",
