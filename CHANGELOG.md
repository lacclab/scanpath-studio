...
- **Data, annotation, comparison and export UI pass** (UX-94) — the Data page is now **Data Management**, its redundant summaries are gone, and local stimulus images sit beside participant metadata. Its dataset table uses integer counts and explicit Demo / Manual / Private / Public kinds; the inspection heading names the active dataset, and column mappings use the add-dataset field-grid grammar. Favorites update the trial picker's star in the same rerun; tags share one compact row; annotation guidance points briefly to **💾 Session**. **📄 Stimulus & Context** now supports general trial context such as titles and instructions. Comparisons use the chosen field as a trial selector, may cross texts, render each trial with its own stimulus words and fixation styling, and no longer repeat the selected plot; their centered control row and trial-only panel headings remove more duplicate copy. Export adds **This trial** to **Trials to Include**, shortens its copy and tooltips, renames **Figure Formats**, and simplifies file naming. Share links always carry the full view, explain host/port changes, and use one **Refresh & Copy** action. The Scanpath selector is labelled, Compare drops its duplicate trial heading, and compared chip rows use fixation-coloured Trial ID chips. The ordinary chip row also drops its repeated participant title. The plot rail now has taller equal-width split toggles with one outline, label-first checkboxes, deterministic named Quick views, a persistent 2×2 Custom view, and a compact three-choice comparison layout control; the fixation snap setting no longer has a separate Linear-reading schematic caption. The reset confirmation's primary action is visible again. Animation transport sits above a slightly larger plot, non-real-time playback is labelled as an illustration, and its setup copy is shorter and clearer. The welcome tour moved into Tutorials, the app keeps a 3rem top inset while trimming bottom whitespace, and Maya Grossman joins the author list.
