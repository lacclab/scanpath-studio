...
### Changed
- **Every mapped field can be cleared in one click** (UX-53)
...
- **Every mapped field can be cleared in one click** (UX-53) — a small **✕** beside each select, shown only while that field has a column, puts it back to `(none)` — the same state as picking `(none)` from the list, not a deletion. That distinction is the whole implementation: the stored value *is* the wire format (`tabs._collect_column_mapping` sweeps every `col_map_*` into the saved config, and `resolve_column_mapping` reads the same keys on views where the editor never renders), so "explicitly not mapped" has to be a value — dropping the key would mean "never answered" and auto-detection would fill the field straight back in. It runs as an `on_click` callback, the only point at which assigning a widget's own key is legal.
