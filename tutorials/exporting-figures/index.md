...
Set the experimental monitor size correctly before adjusting marker sizes or fonts; it defines the figure's coordinate system.
...
PNG and SVG are saved by your browser from the figure on screen, and HTML needs nothing either; PDF, GIF and MP4 need Chrome, Chromium or Edge. For a still figure, the plot's own camera button saves the same PNG.
...
1. select figure and table formats;
1. preview the filename pattern;
1. start the export and inspect one file before using the batch.
