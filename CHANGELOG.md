...
### Changed
- **One control line on the Scanpath view: dataset, trial, scrubber, sort, filter** (UX-64)
- **The datasets are a sortable table, with delete and edit in the row** (UX-54)
...
- **One control line on the Scanpath view: dataset, trial, scrubber, sort, filter** (UX-64) — the view opened with two stacked rows: a Narrow-by row (data source · *Filter by* Text and Participant · **More**) and, under it, the trial picker (selectbox · scrub slider · ◀ ▶ ⇅). They are now one row. Everything that narrows the trial pool moved into a single 🔎 popover at the end of it, so there is one place that filters instead of two — the Text and Participant multiselects sit above the condition and annotation filters that used to live behind **More**. The dataset picker keeps its full width on purpose: you may be comparing two datasets, and its label is what tells them apart, so the scrubber gives up the room and the filter becomes an icon. ➕ **Add data** left this row entirely — the 🗂️ Data page is the only way in now, which is also where the wizard renders. A one-trial pool degrades to dataset · trial · 🔎, since a single-option slider cannot be drawn at all. Compare mode's own picker rows and the multipart screen navigator still have three things on them and keep the pre-UX-64 three-track shape, built by merging two tracks of the same grid so every row's outer boundaries still line up.
- **The datasets are a sortable table, with delete and edit in the row** (UX-54) — the 🗂️ Data page's data-source dropdown is replaced by a table: one row per dataset, sortable by any column and scrollable, carrying **Readers · Trials · Fixations · Words** beside the name and the actions that belong to that dataset — **Open** it, **Edit** its column mapping, **Delete** it. The counts are deliberately cheap (two distinct-counts and two row counts, cached on the frames' fingerprints) because listing the datasets must not cost what opening one does; for the same reason only data already in memory is counted — the open dataset and your own uploads — while a public corpus, which is not read until you open it, shows blank cells and one caption saying why. **Delete restores something UX-64 had broken**: taking the ➕ popover off the Scanpath row removed the only way to remove an uploaded dataset, and the row's ✕ is what puts it back. **Edit** opens the dataset and points at the mapping editor rather than opening a second wizard — "a screen like add-dataset, to edit the maps and see the saved tables" is this page, for whichever dataset is open. The dropdown itself is unchanged on the Scanpath and Corpus views, where it is one cell of the single control line above.
...
- **The add-dataset screen keeps one title row on screen while you scroll** (UX-66) — the top of that page carried four separate pieces of copy written from three different modules — a page header, its one-line summary, a **📂 Data source** subheading, an "Adding a dataset…" caption — and then the wizard repeated the title again. All five are replaced by a single row that **stays put as the fields scroll under it**: *Set up your dataset*, **📘 Show setup guide**, a **📖 Data guide ↗** link, and **✕ Cancel**, which moved onto it from the source-picker block. The long "first time?" paragraph survives as that link's tooltip. The page's top and bottom padding is cut with it, so the first mapping field is visible without scrolling.
