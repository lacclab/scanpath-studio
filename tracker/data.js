 ...
 "items": [
  ...
  },
  {
   ...
   "body": [
    ...
    "",
    "**What's left.** Two kinds of thing, both genuinely optional:",
    "*(a) Polish not yet applied* — `st.column_config.ButtonColumn` for a \"go to",
    "this trial\" button in the Comparisons candidate table and the reader table",
    "(this is a real UX improvement, not just an adoption, so it deserves its own",
    "item rather than being smuggled in here); `st.tabs(height=)`, ",
    "`st.columns(gap=<px>)`, `st.metric(icon=)`; and `App.run()` as a possible",
    "simplification of `cli.launch_app` / `desktop/launcher.py` — that one touches",
    "the desktop bundle, so it needs a desktop smoke-test run to be worth doing.",
    "*(b) The user's review* of the two judgement calls above (fragments,",
    "sidebar lock), since both were the user's items and both came back \"no\".",
    "",
    "**Background (technical).**",
    "",
