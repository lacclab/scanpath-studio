...
- **The three control rows above the plot read as one cluster** (UX-27) — the Narrow-by row's **More**, the trial picker's **◀ ▶ ⇅**, and the chip strip's **Details** / **✏️** are built in three different functions across two modules, each with its own `st.columns` and its own width unit, so they rendered at three heights and two shapes (square icon buttons beside pill-shaped labelled ones). All six triggers now sit in `railbtn_*` containers and take one geometry from `styles.py` — the chip pill, plus a `min-width` that squares up the icon-only ones — and three things were fixed on top of that after looking at the result in a browser: the three rows' trailing controls were **ragged by up to 43 px** and are now flush on one right edge; **◀ and ▶** were a full column gutter apart and now sit 3 px apart as one pair, sharing a column; and the UX-11 sideways nudge on the ✏️ (which was holding it 9.6 px short of that edge) is gone. Purely visual — no behaviour, session key or wire format changed.
