 ...
 "items": [
  ...
  },
  {
   ...
   "body": [
    ...
    "",
    "**What was done.** **Phase 1 only — the inventory the item asks for *before* bulk",
    "edits, plus the deletions it justified.** Measured 2026-08-07 at 90% coverage,",
    "full suite green.",
    "",
    "*Baseline.* **42 409 LOC across 29 first-party modules** (`scanpath_studio/`),",
    "**25 605 LOC across 67 test files**, plus `desktop/` (14 files), `docs/` (39),",
    "`tracker/` (7) and the excluded `other_vis/` (23). Six modules hold **60% of the",
    "package**: `tabs.py` 6 534 · `plots.py` 6 032 · `controls.py` 3 979 · `app.py`",
    "3 376 · `data.py` 2 615 · `wizard.py` 1 702.",
    "",
    "*Dead code and unused dependencies: there are essentially none.* This is the",
    "finding that most changes the shape of the epic — the premise \"too much",
    "historical residue\" does **not** show up as deletable code. `vulture` at 100%",
    "confidence reports only `@st.cache_data` fingerprint parameters (unused *by",
    "design* — they are the cache key) and pytest fixtures. `deptry` reports no",
    "genuinely unused dependency (`pyarrow` is pandas' parquet engine, `watchdog` a",
    "Streamlit runtime dep, the rest are test/docs tooling). At 60% confidence only",
    "**five** functions were truly unreferenced anywhere in the package, tests, docs",
    "or the desktop build, and all five are now deleted:",
    "`annotations.starred_keys` / `keys_with_tag` / `annotated_count`,",
    "`measures.compute_trial_measures` (an unused convenience wrapper), and",
    "`tabs._ordered_trial_ids` — whose docstring described \"the under-image",
    "Prev/Next buttons\", a UI element that no longer exists, i.e. exactly the",
    "implementation archaeology the request names.",
    "",
    "*Dependency direction.* Two import cycles, both narrow and both deliberate-",
    "looking rather than accidental: `measures ⇄ preprocessing` and `data ⇄ datasets`.",
    "Neither is a layering violation across the UI boundary; both are candidates for",
    "the target architecture to resolve or to document as intended.",
    "",
    "*The real hotspots are size and parameter count, not dead paths.* Longest",
    "functions: `controls.sidebar_controls` **1 220 lines / 69 branches**,",
    "`tabs.render_single_trial_tab` 901 / 77, `plots.make_scanpath_animation` 721 / 76,",
    "`plots.make_scanpath_figure` 706 / 102, `app.main` 662 / 68,",
    "`url_state._restore_plot_config` 646 / **141 branches**, `wizard._render_data_setup`",
    "580. Widest signatures: `make_scanpath_figure` **69 parameters**,",
    "`make_scanpath_animation` 53, `make_comparison_figure` 45,",
    "`_make_split_comparison_figure` 44. Those four are one family and they are where",
    "a simplification pass actually pays: a settings *object* threaded through the",
    "builders would collapse ~200 parameters and the matching keyword-forwarding in",
    "`tabs.py`, `api.py`, `cli.py` and `export.py` — which is also the item's",
    "\"consolidate shared behaviour across UI, Share, CLI, API and export instead of",
    "maintaining parallel plumbing by hand\".",
    "",
    "**What's left.** **Everything after the inventory — this item is NOT",
    "finished.** The epic is scoped by the user as one sweeping branch that lands",
    "finished and gets beta-tested, so it was deliberately not started half-way in a",
    "session that was delivering seven other items: an incomplete sweep is the",
    "single worst state for a change whose diff nobody reads. Concretely still to",
    "do: agree the target",
    "architecture; the figure-settings-object refactor across the four builders and",
    "their four call sites; breaking up `sidebar_controls` / `render_single_trial_tab`",
    "/ `main` / `_restore_plot_config`; resolving or documenting the two cycles;",
    "merging one-use modules where that reduces navigation; the comment/archaeology",
    "pass; rewriting tests around retained behaviour; updating `AGENTS.md` +",
    "`scanpath_studio/CLAUDE.md`; and the full lint/test/docs/desktop-smoke gates with",
    "before/after numbers against the baseline recorded above.",
    "",
    ...
    "full lint/test/docs/desktop smoke gates.",
    "",
    "*How this is run* (user, 2026-08-07): **delegated end to end** — the target architecture",
    "does not come back for sign-off, and the user will not read the diff. It lands as **one",
    "sweeping branch**, and the user beta-tests the finished app instead. That moves the whole",
    "safety burden onto the automated gates and onto behaviour parity: the full test suite, lint,",
    "docs build and the desktop smoke test all have to pass, and anything a test does *not* pin",
    "is unprotected — so pin retained behaviour with tests before deleting the code that",
    "implements it, not after. Related: **ENG-5**, **ENG-6**, **ENG-11**,",
    "**ENG-18**, **ENG-24**."
   ]
  },
  {
