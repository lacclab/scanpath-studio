...
### Added
- **Reset settings** (UX-26) — a **♻️ Reset settings** popover at the top of the rail puts the visualization settings, the trial filters, or both back to the app's defaults in one click (annotations, column mapping, data source and the selected trial are kept); it also strips the viz parameters from the URL, so the reset sticks on a page opened from a Share link. The filter half also has a permanent **✕ Clear all filters** button at the foot of the **More** filter popover, instead of only appearing once a filter left nothing.
