 ...
 "items": [
  ...
  },
  {
   ...
   "body": [
    ...
    "cluster instead of three independently-sized rows. To be implemented after",
    "**ENG-36**.",
    "",
    "**What was done.** One shared CSS hook, as the note below suggested, rather",
    "than hand-tuning three call sites. All six triggers — **More**, **◀**, **▶**,",
    "**⇅**, **Details**, **✏️** — now render inside a container keyed `railbtn_*`,",
    "and `styles.py` gives `[class*=\"st-key-railbtn_\"] button` a single geometry:",
    "the **chip pill** (`border-radius: 999px`, `padding: .1rem .65rem`,",
    "`font-size: .9rem`, `min-height: 0`) plus `min-width: 2.3rem`. The chip pill",
    "was chosen because it was already tuned against the chip strip *and* it is the",
    "smallest of the three shapes, so adopting it shrinks the cluster instead of",
    "growing it; the `min-width` is what squares up ◀ ▶ ⇅, which otherwise each",
    "collapse to their own glyph width. The two `width=\"stretch\"` arguments on the",
    "step buttons are gone (that stretch-inside-a-0.55-column is what made them",
    "square in the first place), and the pre-existing `chip_edit_box` /",
    "`chip_details_box` keys were renamed into the `railbtn_*` family so there is",
    "one rule, not two that must be kept in sync. UX-11's baseline nudge for the",
    "chip pair survives as a separate, narrower rule.",
    "",
    "**What's left.** **The user's eye — this item is only judgeable on screen.**",
    "The app boots clean and the full suite passes, but nothing here is testable:",
    "`AppTest` has no DOM, and the Chrome extension was not connected in the",
    "implementing session, so the aligned cluster has not actually been *looked* at.",
    "Worth checking: that the pill height reads right for **More** (the widest",
    "label), that ◀ ▶ ⇅ now match each other and sit level with the row above, and",
    "that nothing regressed at a narrow rail width, where the Quick-view buttons",
    "already drop to emoji-only (UX-29).",
    "",
    "**Background (technical).** Each row is built independently with its own",
    "`st.columns` call and its own width unit, which is why nothing lines up",
    "today: the **More** popover trigger sits at",
    "[`tabs.py:2411-2423`](scanpath_studio/tabs.py:2411) (`nb_source, nb_label,",
    "nb_text, nb_part, more_col = st.columns([2.2, 0.9, 2.2, 2.2, 1.1])`); the ◀ ▶",
    "step buttons + ⇅ sort popover are",
    "[`utils.py:480-550`](scanpath_studio/utils.py:480) (`sel_col, slider_col,",
    "prev_col, next_col, sort_col = host.columns([3, 5, 0.55, 0.55, 0.55])`, each",
    "button `width=\"stretch\"` inside its own narrow column — this is what makes",
    "the ◀ ▶ pair square while **More**/**Details** stay pill-shaped); and the",
    "**Details** / **✏️ Edit chips** pair sits at",
    "[`tabs.py:2860-2873`](scanpath_studio/tabs.py:2860) (`strip_col, details_col,",
    "edit_col = st.columns([11, 1.6, 0.9])`, the pencil trigger further squeezed",
    "to chip size by the `chip_edit_box` keyed container in `styles.py`). Fixing",
    "this means picking one consistent button size/shape (height, corner radius,",
    "icon-only vs. label+chevron) and applying it across all three rows — likely",
    "via a shared CSS hook in `styles.py` keyed off container names, since the",
    "buttons are built in three different functions across two modules and don't",
    "share a helper today. Purely visual — no behavior, session-state key, or",
    "wire-format change, so this doesn't touch the four-surface rule. Depends on",
    "**ENG-36** landing first per the user's sequencing call (2026-08-07); related:",
    "**UX-11** (the chip-row layout), **VIZ-31** (the rail's other button styling",
    "conventions)."
   ]
  },
  {
  ...
  },
  {
   ...
   "body": [
    ...
    "",
    "**What's left.** Nothing. Scope note: per-scanpath compare styling isn't on",
