...
- **Data, annotation, comparison and export UI pass** (UX-94) — the Data page is now **Data Management**, its redundant summaries are gone, and local stimulus images sit beside participant metadata. Its dataset table now carries the complete Participants / Texts / Trials / Fixations / Words / Gaze points / Screens summary, integer counts, explicit Demo / Manual / Private / Public kinds, and per-row Rename / Remove actions; the inspection heading names the active dataset, and column mappings use the add-dataset field-grid grammar. Stored-dataset editing now reuses the add flow's three-column Recording setup controls too, while built-in setup summaries use the same field-card grammar. Favorites update the trial picker's star in the same rerun; tags share one compact row; annotation guidance points briefly to **💾 Session**. **📄 Stimulus & Context** now supports general trial context such as titles and instructions. Comparisons use the chosen field as a trial selector, may cross texts, render each trial with its own stimulus words and fixation styling, and no longer repeat the selected plot; their centered control row and trial-only panel headings remove more duplicate copy. Scanpath B now has an independent 🔎 filter even when both trials use the active dataset. Export adds **This trial** to **Trials to Include**, shortens its copy and tooltips, renames **Figure Formats**, and simplifies file naming. Share links always carry the full view, explain host/port changes, and use one compact **Refresh & Copy** action. The bold Scanpath dataset label, Compare's removed duplicate heading, and fixation-coloured Trial ID chips strengthen selection hierarchy. The ordinary chip row also drops its repeated participant title. The plot rail uses equal-width split toggles with one outline; the latest sizing pass reduces their height slightly while making the full dropdown segment clickable. It also has label-first checkboxes, deterministic named Quick views, a persistent 2×2 Custom view, and a compact three-choice comparison layout control; the fixation snap setting no longer has a separate Linear-reading schematic caption. Stimulus typography uses a concise box-scaling or px/pt unit flow and no longer crashes when Text is off; Heatmap and Raw gaze use contextual labels without repeated layer names. Corpus figure settings stay inside their collapsed style panel. The reset confirmation's primary action is visible again. Animation transport sits above a slightly larger plot, non-real-time playback is labelled as an illustration, and its setup copy is shorter and clearer. The welcome tour moved into Tutorials, the app keeps a 3rem top inset while trimming bottom whitespace, and Maya Grossman joins the author list.

- **Session now distinguishes local recovery from portable backup** (UX-96) — three card-like sections replace the mixed list. **Automatic recovery** reports the saved datasets, rows, annotations, size and folder; states exactly what is included; can be paused; and can clear the saved copy without closing the current work. **JSON backup** downloads/restores settings, selection, mappings, metadata attachments and annotations while stating that uploaded dataset rows are not included. **Debug tools** keeps diagnostics opt-in and beside the other session-level state. The bundled-demo and computation-rebuild escape hatches remain available in a compact Troubleshooting disclosure.
...
### Fixed
- **A tutorial no longer opens on the page you were already on and asks you to navigate** (UX-83)
...
- **A column mapping the pipeline rejects no longer takes the whole app down** (BUG-28) — mapping `screen_id` on the fixations report and not on the words report replaced the entire app with a `ValueError` traceback ("Multipart identity is present in only one report…"), and the controls that could undo it were on the far side of the exception: the Column-mapping panels are written *during* the run that died, and the 🗂️ Data page they live on is hidden while another view is active — so from the Scanpath view the crash left the top nav and nothing else. The refusal itself is correct (with a screen id on one side only, every fixation would be matched against every screen's word boxes); treating it as fatal to the session was not. A rejected mapping now recovers exactly like an incomplete one — the error and its reason on the Data page, the raw tables under it, the mapping panels still editable, and a signpost from whichever view you were on — plus **↩️ Reset to the auto-detected mapping**, which drops every stored `col_map_*` pick, including one restored from the on-device recovery cache. The upload wizard's two paths (generic and the MultiplEYE preset) do the same: the wizard stays up with the reason against its review step and **✅ Add dataset** disabled. The exception is logged with its traceback, so the 🐛 Debug panel and the terminal keep the full story. Nothing was ever cached that had to be cleared by hand — `st.cache_data` does not store exceptions — but the stored mapping could survive a refresh, which the reset button now handles.
  ...
  Two escape hatches that don't route through the 🗂️ Data page came with it, because being sent to a page to fix a dataset is not always what you want — sometimes you just want something that draws. The signpost now offers **🧪 Load the bundled demo** beside *Go to Data setup* (it switches source *and* re-detects the mapping, so it works even when the wedged dataset is the demo), and the 💾 Session menu grew a **🧹 Start fresh** block with the same button plus **🧹 Clear cached computations**. The Session menu's contents are now written *before* the data loads rather than in `main`'s epilogue: every path where the dataset can't be drawn returns early, so the menu you open when stuck used to show its headings with no controls under them — including 🗑 Forget saved session, the one control that clears what a refresh would restore.
...
- **A tutorial no longer opens on the page you were already on and asks you to navigate** (UX-83) — starting a tutorial whose first step names a view other than Scanpath (a Data or Corpus Analysis step) used to leave the card open wherever you already stood, offering a **Show me / Open this panel** button to get there — `_start_use_case` recorded where Exit should return to but never navigated. It now opens the current step's page (index 0 on a fresh start, wherever progress left off on resume) the moment the tutorial starts. `tutorial_return` is still captured first, so Exit still lands back where you actually began.
#### Changed
...
- **Deleting a dataset drops the computations derived from it** (UX-87) — every figure, table and normalization cached for a dataset is dead the moment its frames leave the session, so `_remove_dataset` now clears the computation cache rather than leaving stale results in memory one undo-less click away from being attributed to whatever takes the name next. `@st.cache_data` has no per-key eviction, so this is the same lossless instrument the 🧹 *Clear cached computations* button uses — survivors recompute on the next rerun from frames that are still loaded. The standalone button stays: BUG-28 put it in 💾 Session as the escape hatch for a session whose column mapping is wedged, and there is no delete-all-datasets button to fold it into.
