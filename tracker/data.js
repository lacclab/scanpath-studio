...
window.TRACKER = {
 ...
 "items": [
  ...
  },
  {
   ...
   "archived": false,
   "body": [
    "> **Implemented 2026-07-29 — pending approval.** All 14 targeted cells in the VIZ-21",
    "> mapping table moved ❌→✅, across three layers.",
    ">",
    "> **Drift correction on all three paths** — the headline item. `alignment.correct` is",
    "> now applied **once** in `tabs.render_single_trial_tab`, above the",
    "> `if animate / elif comparing / else` split, via a new `tabs._drift_corrected`; the",
    "> *\"Off\"* path returns the **same frame object**, so the default is a true no-op that",
    "> busts no cache and changes no figure. The comparison path corrects **both**",
    ...
    "> `fixation_flags` and the three colour-bar style args. Two design calls worth",
    "> knowing: the arrows and the PRE-2 *Highlight* overlays are full-length traces whose",
    "> positions un-mask as the replay reaches them, so an arrow appears **with its own",
    "> saccade** rather than all at frame zero; and *Discard* filters rows **after** the",
    "> axis ranges are computed (as the static figure does), so filtering never re-frames",
    "> the view.",
    ">",
    "> **`make_comparison_figure` / `_make_split_comparison_figure`** gained",
    "> `fixation_symbol` (on both marker branches and both split layouts),",
    "> `highlight_column` — which finally makes the `highlight_text_color` they already",
    "> accepted do something instead of being a dead parameter — the four `background_image*`",
    "> args (overlay **and** per-panel in both split layouts, so there is no split-layout",
    "> gap) and colour-bar styling.",
    ">",
    "> **Controls + table re-synced.** Every cell was re-verified against `plots.py` and",
    "> `tabs.py` *before* its gate was touched — nothing taken on trust from the hand-off",
    "> notes. Three controls that had been sharing a gate with a now-live neighbour were",
    "> split onto their own so they stay honestly disabled: the drift **connectors**,",
    "> **Snap fixations above words** (it was reusing `align_disabled` despite being the",
    "> VIZ-9 schematic, not drift correction) and the **Mark border** span colour. The span",
    "> *Style* radio stays **enabled** with a ⚠️ in its help, since one of its two options",
    "> is now live everywhere. Verified across all four mode combinations that 49",
    "> pre-seeded `global_*` / `single_*` values survive byte-identical.",
    ">",
    "> One deliberate, documented behaviour change: the animation's and Compare's",
    "> previously-bare colour bars now go through `plots._colorbar_dict`, so their",
    "> position/thickness/tickfont match the static figure.",
    ">",
    "> **Still ❌ and correctly gated:** heatmap, raw gaze, X/Y field, saccade-class",
    "> colouring, Arc + snap in the animation, `show_fix`, `order_font_color` in Compare —",
    "> those builders take no such parameter. Follow-ups filed: **VIZ-24**, **VIZ-25**,",
    "> **ENG-23**, **ENG-24**.",
    "",
  ...
  },
  {
   ...
   "archived": false,
   "body": [
    "> **Implemented 2026-07-29 — pending approval.** `plots._arch_apex_y` computes the",
    "> topmost point of the drawn arch in **closed form** — the quadratic Bézier's",
    "> extremum, derived rather than sampled — over the same curve `_arch_control_point`",
    "> defines, and the Arc headroom reservation now uses it instead of the *t*=0.5 point.",
    ">",
    "> On the regression case the reserved top moved 2.40 → −13.03 (i.e. it now actually",
    "> covers the peak); a level arc reserves exactly what it did before, so no existing",
    "> figure moves.",
    "",
  ...
  },
  {
   ...
   "archived": false,
   "body": [
    "> **Implemented 2026-07-29 — pending approval, with one surface still open.**",
    "> `render` gained `--drift-correction ALGORITHM` and `--drift-connectors`, mapped",
    "> onto `api.plot_scanpath`'s existing parameters (defaults `None`/`False` → today's",
    "> uncorrected figure, unchanged). The name is validated against `alignment.ALGORITHMS`",
    "> through an argparse `type=` callable, so a typo exits 2 listing all ten valid names",
    "> and never reaches the API; the callable imports `alignment` lazily so `render --help`",
    "> doesn't pay scipy's startup. Names are accepted case-insensitively, so the app's",
    "> title-cased spelling (\"Warp\") works verbatim.",
    ">",
    "> `api.animate_scanpath` takes neither parameter (PRE-3 is static-only), so with",
    "> `--animate` both are reported through the existing \"not supported with --animate,",
    "> ignoring:\" warning rather than silently dropped. `docs/cli.md` gained the two flag",
    "> rows and an example.",
    ">",
    "> **⚠️ Not fully closed.** Drift correction is *still* missing from the **deep link /",
    "> Share** surface: `global_align_algorithm` / `global_align_connectors` are neither",
    "> seeded by `_URL_PRESETS` / `_apply_url_preset` nor emitted by `_build_share_query`,",
    "> so a corrected view can't be shared or restored. Per *Exposing a feature on every",
    "> surface* in [`AGENTS.md`](AGENTS.md) this item is only done when that lands —",
    "> tracked as **ENG-23**. This gap became more visible with **VIZ-23**, which made",
    "> drift correction apply on two more render paths.",
    "",
  ...
  },
  {
   ...
   "archived": false,
   "body": [
    "The last leg of the four-surfaces contract for **PRE-3** drift correction, found",
    "while closing **ENG-22** (which added the CLI leg).",
    "",
    "`global_align_algorithm` (stored title-cased, e.g. `\"Warp\"`; `\"Off\"` = disabled) and",
    "`global_align_connectors` (bool) appear in **neither**",
    "[`url_state.py`](scanpath_studio/url_state.py)'s `_URL_PRESETS` / `_apply_url_preset`",
    "(read) **nor** `_build_share_query` (write). So a drift-corrected view cannot be",
    "linked to or restored — the recipient of a Share link silently gets the uncorrected",
    "figure, which is a *different scientific claim* about where the reader looked.",
    "",
    "**VIZ-23** raised the stakes: drift correction now applies on the animation and",
    "comparison paths too, so the gap covers three render modes rather than one.",
    "",
    "Wire both keys into `_URL_PRESETS` + `_apply_url_preset` and the inverse",
    "`_build_share_query`, validating the algorithm against `alignment.ALGORITHMS`",
    "(the CLI accepts names case-insensitively, so a title-cased value from a link works",
    "verbatim). Then add them to [`session_keys.py`](scanpath_studio/session_keys.py) and",
    "its frozen list, so `tests/test_session_key_contract.py` guards them from that point",
    "on — **ENG-6** built exactly this gate for exactly this reason."
   ]
  },
  {
   "id": "VIZ-24",
