...
### Fixed
- **The app boots again: a stale argument on the trial picker** (BUG-23)
...
- **The app boots again: a stale argument on the trial picker** (BUG-23) — the Scanpath view raised `TypeError: select_trial() got an unexpected keyword argument 'filter_cols'` before the trial picker rendered, so the whole view was a traceback. BUG-16 deliberately stopped pruning composite trial-id components that are also **More** filter columns and removed the parameter; the call site in `tabs.py` and a paragraph of `utils.select_trial`'s docstring were left behind pointing at it. Both are gone; the AppTest suite is back to 119/119 from 9.
